...
"variable capture" or just [**capture**]{.smallcaps} for short. The `y`
that we applied to the summing function was captured by the binding site
of the nested lambda.
...
[**capture-avoiding substitution**]{.smallcaps}. There are many internal
representations we can pick from to achieve this. The strategy above
where variables were simple strings is called [**explicit
variables**]{.smallcaps}.
...
Being able to rename bound variables at will is called
[**α-varying**]{.smallcaps}, and when two terms can be made identical by
just α-varying them, we say they're [**α-equivalent**]{.smallcaps}.
...
we call these indices [**de Bruijn indices**]{.smallcaps}:
...
going under a binder is called [**lifting**]{.smallcaps} (or sometimes,
[**shifting**]{.smallcaps}).
...
the expense of free variables. [**Locally nameless terms**]{.smallcaps}
solve this by giving free variables explicit names, but using indices
instead of names for bound (or "local") variables, thus the
name.[^globally-nameless]
