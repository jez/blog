...
One of the best parts of fzf is that you can script it. In the screen cast above,
you see how I'm using `**<TAB>` to trigger fzf to list all bazel targets. It uses the
[Custom fuzzy completion][api] API to register a shell function to run on `TAB`.[^kill]
...
You can see one caveat in the comment there: it doesn't do all that well at
...
anything in `//...`, they don't show up in the output.[^caveat]
